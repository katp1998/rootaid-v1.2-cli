import { generateToken, validatePassword, generatePassword } from "../utils/index";
import {
  findUser,
  createUser,
} from "../database/repository/user.repository";

const userFind = async (email:string) => {
    return await findUser(email);
}

const registerUser = async (name: string, email: string, password: string) => {
  try {
    //CHECK IF USER EXISTS (user.repository):
...
export { registerUser, loginUser, userFind };